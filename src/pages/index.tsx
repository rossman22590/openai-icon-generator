      ...
      fetch(url, {
        ...
        .then((data) => {
          ...
          for (let i = 0; i < arraySize; i++) {
            if (imageContainer) {
              console.log('image el is available');
              imageContainer.innerHTML += `<img src="${data.data[i].url}" alt="image" class="w-100 p-2">`;
            } else {
        ...
        <form action="/send-data-here" method="post" className="flex flex-col items-center">
          ...
          <>
            <div className={`mt-12 results-container container rounded-lg flex justify-center items-center w-16 h-16 ${backgroundColor}`}>
              <div ref={imageContainer} className="fill-current w-4 h-4 mr-2"></div>
            </div>
            <button 
